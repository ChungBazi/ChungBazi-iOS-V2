...
Airbnb Swift Style Guide 기반의 iOS SwiftUI 프로젝트(ChungBazi) 코딩 가이드라인입니다.
...
> TCA를 사용하는 Feature의 Action 네이밍도 이 규칙을 그대로 따릅니다. 자세한 내용은 [TCA_GUIDELINES.md §3](./TCA_GUIDELINES.md#3-action)을 참고하세요.
...
> `BaziDomain`, `BaziData` 등 모듈 경계를 넘나드는 타입은 특히 이 규칙이 중요합니다. Tuist 멀티모듈 구조에서 `public`은 곧 "다른 모듈에 노출되는 API"를 의미하므로, 의도치 않게 넓어지는 접근 범위는 모듈 간 결합도를 그대로 키웁니다.
...
> TCA Feature의 세부 파일 구조·네이밍 규칙(`~Feature.swift`, `~View.swift` 등)은 [TCA_GUIDELINES.md §1](./TCA_GUIDELINES.md#1-파일-구조)에서 별도로 다룹니다.
...
- [TCA_GUIDELINES.md](./TCA_GUIDELINES.md) — `BaziPresentation` 모듈의 TCA(The Composable Architecture) 컨벤션

...
public struct CategoryPolicyClient: Sendable {
    public var fetchPolicies: @Sendable (_ category: PolicyCategory, _ sort: String, _ cursor: String?, _ size: Int) async throws -> PolicyPageVO
    public var fetchPersonalized: @Sendable (_ category: PolicyCategory) async throws -> [PolicySummary]
}
...
extension CategoryPolicyClient: TestDependencyKey {
    ...
    public static let previewValue = CategoryPolicyClient(
        ...
        fetchPersonalized: { _ in Array(PolicySummary.mockList.prefix(3)) }
    )
}

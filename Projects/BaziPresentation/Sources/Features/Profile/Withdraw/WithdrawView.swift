...
extension WithdrawView {
    ...
    private func checkboxRow(title: String, suffix: String? = nil, isSelected: Bool, action: @escaping () -> Void) -> some View {
...
extension WithdrawView {
    ...
    private var confirmAlertOverlay: some View {
        ZStack {
            ...
            BZAlert(
                ...
                confirmTitle: "탈퇴하기",
                confirmType: .accent,
                onCancel: { store.send(.didCancelConfirm) },
                onConfirm: { store.send(.didConfirmWithdraw) },
                onClose: { store.send(.didCancelConfirm) }
            )
